								...
								const char *binfile)
{
  ...
  cellarray_nvec *lattice1 = gridlink_nvec(ND1, X1, Y1, Z1, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, rpmax, rpmax, bin_refine_factor, bin_refine_factor, bin_refine_factor, &nmesh_x, &nmesh_y, &nmesh_z);
  ...
  cellarray_nvec *lattice2 = NULL;
  if(autocorr==0) {
	int ngrid2_x=0,ngrid2_y=0,ngrid2_z=0;
	lattice2 = gridlink_nvec(ND2, X2, Y2, Z2, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, rpmax, rpmax, bin_refine_factor, bin_refine_factor, bin_refine_factor, &ngrid2_x, &ngrid2_y, &ngrid2_z);
...
#pragma omp for  schedule(dynamic) 
		...
		for(int64_t index1=0;index1<totncells;index1++) {
		  ...
		  cellarray_nvec *first = &(lattice1[index1]);
		  ...
		  for(int iix=-bin_refine_factor;iix<=bin_refine_factor;iix++){
				...
				for(int iiy=-bin_refine_factor;iiy<=bin_refine_factor;iiy++){
				  ...
				  for(int iiz=-bin_refine_factor;iiz<=bin_refine_factor;iiz++){
						...
						const cellarray_nvec *second = &(lattice2[index2]);
						DOUBLE *x1 = first->pos;
