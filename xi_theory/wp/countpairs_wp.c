            ...
            for(int iix=-bin_refine_factor;iix<=bin_refine_factor;iix++) {
                ...
                for(int iiy=-bin_refine_factor;iiy<=bin_refine_factor;iiy++) {
                    ...
                    for(int iiz=0;iiz<=zbin_refine_factor;iiz++) {
                        ...
                        const cellarray_index *second = &(lattice[index2]);
                        if(second->nelements == 0) {
                            continue;
                        }
            

                        different_cell_wp_kernel(first, second,
                                                 X, Y, Z,
                                                 sqr_rpmax, sqr_rpmin, nbin, rupp_sqr, pimax, off_xwrap, off_ywrap, off_zwrap
#ifdef USE_AVX
                                                 ,m_rupp_sqr
#ifdef OUTPUT_RPAVG
                                                 ,m_kbin
