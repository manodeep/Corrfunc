...
unsigned int read_positions(const char *filename, const char *format, void **xpos, void **ypos, void **zpos, const size_t size)
{
	...
	if(strncmp(format,"f",1)==0) {
		...
		//TO DO -- cover the case where the file is written in the other precision. 
		...
		if(dummy == size) {
			...
			if(dummy == 4) {
				...
				for(unsigned int i=0;i<np;i++) tmp_x[i] = tmp[i];

				//read-in y
				my_ftread(tmp, dummy, np, fp);
				for(unsigned int i=0;i<np;i++) tmp_y[i] = tmp[i];

				//read-in z
				my_ftread(tmp, dummy, np, fp);
				for(unsigned int i=0;i<np;i++) tmp_z[i] = tmp[i];
			...
			if(size==8) {
				...
				if(nread == nitems) {
					if(isfinite(dbl_x) && isfinite(dbl_y) && isfinite(dbl_z)){
						...
						fprintf(stderr,"WARNING: Nans found in data for i=%d file = `%s'\n",i,filename);
					}
				}
			} else {
