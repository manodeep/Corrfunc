                                 ...
                                 int *nlattice_z)
{
    ...
    /*
      Allocate memory for each of the fields in cellarray_index. Since we haven't processed the data yet,
      expected_n is a reasonable guess as to the number of points in the cell.
    */
    ...
    for (int64_t i=0;i<np;i++)  {
        ...
        int index = ix*nmesh_y*nmesh_z + iy*nmesh_z + iz;
        cell_index[i] = index;
    }
...
#undef MULTIPLE_ARRAY_EXCHANGER
    ...
    assert(cell_index[0] == start_cell && "First cell index must be 0");
    for(int64_t i=1;i<np;i++) {
        const int64_t icell = cell_index[i];
        if(icell != start_cell) {
            /* lattice[start_cell].nelements = i - lattice[start_cell].start; */
