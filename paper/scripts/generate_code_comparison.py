...
def plot_results(timings_fn):
    ...
    seaborn.set_context('paper', rc={"lines.linewidth": linewidth, "axes.linewidth":axeslinewidth})
    #seaborn.set(font_scale=1.5)

    _func_runtimes = dict(np.load(timings_fn))
    all_np = _func_runtimes.pop('all_np')

    # In the serial case, the legend is too big and really difficult to fit into the
    # plot. Therefore, changing the actual names on the functions on the legend
    # to allow bigger font sizes...
    if not parallel:
        print("Before replacement: the keys are: {}".format(_func_runtimes.keys()))
        func_key_name_replacements = {'scikit-learn KDTree':'sklearn KDTree',
                                      'mlpack RangeSearch': 'mlpack'}
        for oldkey, newkey in func_key_name_replacements.items():
            assert oldkey in _func_runtimes.keys()
            v = _func_runtimes[oldkey]
            _func_runtimes[newkey] = v
            _func_runtimes.pop(oldkey)

        print("After replacement: the keys are = {}".format(_func_runtimes.keys()))


    # Assign a persistent line style to each func
    order = ['halotools', 'kdcount', 'Treecorr', 'CUTE_box', 'sklearn KDTree', 'SciPy cKDTree', 'mlpack', 'swot']
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.tick_params(width=majortickwidth, length=majorticklen, pad=tickpad, which='major')
    ax.tick_params(width=minortickwidth, length=minorticklen, which='minor')
    ax.tick_params(pad=tickpad, which='both')
    ...
    ax.legend(framealpha=1.0, fancybox=True)
    plt.setp(ax.get_legend().get_texts(), fontsize=legendfontsize) # for legend text
    for item in ([ax.title, ax.xaxis.label, ax.yaxis.label] +
                 ax.get_xticklabels() + ax.get_yticklabels()):
            item.set_fontsize(axisfontsize)
